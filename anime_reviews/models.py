...
class DeepConcatenationColaborativeFilteringModel(pl.LightningModule):
    ...
    class PrintingCallback(pl.Callback):
        ...
        def on_train_start(self, trainer: pl.Trainer, model: pl.LightningDataModule) -> None:
            ...
            LOGGER.info(f'Number of Users: {model.hparams.number_of_users:,}')
            LOGGER.info(f'Embedding Size: {model.hparams.embedding_size:,}')
            LOGGER.info(f'Dense Layer Count: {model.hparams.dense_layer_count:,}')
            LOGGER.info(f'Regularization Factor: {model.hparams.regularization_factor:,}')
            LOGGER.info(f'Dropout Probability: {model.hparams.dropout_probability:,}')
            LOGGER.info('')
            LOGGER.info('Data:')
    ...
    @classmethod    
    def train_model(cls, gpus: List[int], number_of_epochs: int, batch_size: int, gradient_clip_val: float, embedding_size: int, dense_layer_count: int, regularization_factor: float, dropout_probability: float) -> float:
    
        checkpoint_dir = cls.checkpoint_directory_from_hyperparameters(learning_rate, number_of_epochs, batch_size, gradient_clip_val, embedding_size, dense_layer_count, regularization_factor, dropout_probability)
        if not os.path.isdir(checkpoint_dir):
            os.makedirs(checkpoint_dir)
        checkpoint_callback = pl.callbacks.ModelCheckpoint(
                filepath=os.path.join(checkpoint_dir, 'checkpoint_{epoch:03d}_{val_checkpoint_on}'),
